...
### 기존에 있던 객체와 똑같으면서 독립적인 객체를 만들고 싶을때

- for - in 문 사용하여 복제

정말 복제가 필요한 상황이라면 새로운 객체를 만든 다음 기존 객체의 **프로퍼티들을 순회**해 **원시 수준까지 프로퍼티를 복사**하면 됩니다.
...
```js
// Object.assign(목표 객체, 복사하고자하는 객체)
...
Q. 얕은 복사에서 중첩 객체를 처리 못한다는 의미는?

- 이중으로 참조되었을때 이중 참조 sizes.height 을 복사해내지 못한다는 의미인지
- user을 clone 으로 얕은 복사를 했을때 복사된 객체 clone을 수정하면 원본user도 함께 수정된다는 의미인지
